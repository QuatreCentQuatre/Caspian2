...
class Image
{
    ...
    /* Position */
    const LEFT_TOP     = 'LT';
    const LEFT_BOTTOM  = 'LB';
    const RIGHT_TOP    = 'RT';
    const RIGHT_BOTTOM = 'RB';
    ...
    public function crop($x=0, $y=0, $layer=0)
    {
        ...
        if ($layer == 0) {
            $this->layers->cropMaximumInPercent($x, $y, self::LEFT_TOP);
            return $this;
        }

        $this->layers->layers[intval((string)$layer)]->cropMaximumInPercent($x, $y, self::LEFT_TOP);
        return $this;
    }
     ...
     *
     */
    public function flip($direction=self::HORIZONTAL, $layer=0)
    {
        if (empty($this->layers)) { return $this; }
     ...
     *
     */
    public function addLayer($file, $x=0, $y=0, $position=self::LEFT_TOP)
    {
        if (empty($this->layers)) { return $this; }
    ...
    public function outputFile($target, $quality=80, $type=null, $color=null, $output_hash=false)
    {
        ...
        $upload = new Upload;
        $hash   = $upload->uploadImage($file, $target, 'cache');
    ...
    {
        $hash = $width . 'x' . $height . '_' . md5($file);

        /* Hash given */
        $upload = new Upload;
        $cached = $upload->isCached($hash);
        ...
        if (!$cached) {
            ...
            $file = $upload->getCache($upload->addCache($src, $hash));
            unlink($src);
        } else {
            $file = $upload->getCache($hash);
        }
