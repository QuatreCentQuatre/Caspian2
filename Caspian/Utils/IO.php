...
class IO extends Base
{
    ...
    public function __construct($file, $open=false)
    {
        ...
        if (file_exists($file)) {
            ...
            /* Basics */
            $this->file = $file;
            $this->size = $this->get_size($stats['size']);
            
            /* Access and last modification time */
            ob_start();
            format_date($stats['mtime'], true, $_SESSION['language']);
            $this->mtime = ob_get_clean();
            
            ob_start();
            format_date($stats['atime'], true, $_SESSION['language']);
            $this->atime = ob_get_clean();
